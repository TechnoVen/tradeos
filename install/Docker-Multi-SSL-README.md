# TradeOS Advanced Docker Installation (Multi-Instance & Custom SSL)
...
- **Multiple TradeOS instances** on a single server.
...
```bash
wget https://raw.githubusercontent.com/marketcalls/tradeos/main/install/install-docker-multi-custom-ssl.sh
...
1.  **Instance Name**:
    ...
    - This allows you to run multiple independent copies of TradeOS side-by-side.
...
```
/opt/
  ├── tradeos-algo1/       # Instance 1
  │   ├── docker-compose.yaml
  │   └── .env
  └── tradeos-fyers-bot/   # Instance 2 (Different Broker/Strategy)
      ├── docker-compose.yaml
      └── .env
```
