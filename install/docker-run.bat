@echo off
REM ============================================================================
REM TradeOS Docker Runner for Windows
REM ============================================================================
REM
REM Quick Start (2 commands):
REM   1. Download: curl.exe -O https://raw.githubusercontent.com/marketcalls/tradeos/main/install/docker-run.bat
REM   2. Run:      docker-run.bat
REM
REM Commands:
REM   start    - Start TradeOS container (default, runs setup if needed)
...
REM Configuration
set IMAGE=marketcalls/tradeos:latest
set CONTAINER=tradeos
set ENV_FILE=.env
set SAMPLE_ENV_URL=https://raw.githubusercontent.com/marketcalls/tradeos/main/.sample.env
REM Use the directory where the script is located
set TRADEOS_DIR=%~dp0
REM Remove trailing backslash
if "%TRADEOS_DIR:~-1%"=="\" set TRADEOS_DIR=%TRADEOS_DIR:~0,-1%
set SETUP_FAILED=0
...
:setup
echo [INFO] Setting up TradeOS in %TRADEOS_DIR%...
echo.

REM Create db directory
if not exist "%TRADEOS_DIR%\db\" (
    echo [INFO] Creating database directory...
    md "%TRADEOS_DIR%\db" 2>nul
    if errorlevel 1 (
...
if exist "%TRADEOS_DIR%\%ENV_FILE%" (
    echo [WARNING] .env file already exists at %TRADEOS_DIR%\%ENV_FILE%
...
if errorlevel 1 (
    ...
    powershell -Command "Invoke-WebRequest -Uri '%SAMPLE_ENV_URL%' -OutFile '%TRADEOS_DIR%\%ENV_FILE%'" 2>nul
) else (
    curl.exe -sL "%SAMPLE_ENV_URL%" -o "%TRADEOS_DIR%\%ENV_FILE%" 2>nul
)

REM Check if download succeeded
if not exist "%TRADEOS_DIR%\%ENV_FILE%" (
    echo [ERROR] Failed to download configuration template!
    echo Please check your internet connection.
    echo.
    echo Manual setup:
    echo   1. Download .sample.env from https://github.com/marketcalls/tradeos
    echo   2. Save it as %TRADEOS_DIR%\.env
    echo   3. Run this script again
    set SETUP_FAILED=1
    goto setup_end
)
...
powershell -Command "(Get-Content '%TRADEOS_DIR%\%ENV_FILE%') -replace '3daa0403ce2501ee7432b75bf100048e3cf510d63d2754f952e93d88bf07ea84', '%APP_KEY%' | Set-Content '%TRADEOS_DIR%\%ENV_FILE%'"
powershell -Command "(Get-Content '%TRADEOS_DIR%\%ENV_FILE%') -replace 'a25d94718479b170c16278e321ea6c989358bf499a658fd20c90033cef8ce772', '%API_KEY_PEPPER%' | Set-Content '%TRADEOS_DIR%\%ENV_FILE%'"
...
powershell -Command "(Get-Content '%TRADEOS_DIR%\%ENV_FILE%') -replace 'BROKER_API_KEY = ''YOUR_BROKER_API_KEY''', 'BROKER_API_KEY = ''%BROKER_API_KEY%''' | Set-Content '%TRADEOS_DIR%\%ENV_FILE%'"
powershell -Command "(Get-Content '%TRADEOS_DIR%\%ENV_FILE%') -replace 'BROKER_API_SECRET = ''YOUR_BROKER_API_SECRET''', 'BROKER_API_SECRET = ''%BROKER_API_SECRET%''' | Set-Content '%TRADEOS_DIR%\%ENV_FILE%'"

REM Update redirect URL with broker name (replace <broker> placeholder)
powershell -Command "(Get-Content '%TRADEOS_DIR%\%ENV_FILE%') -replace '<broker>', '%BROKER_NAME%' | Set-Content '%TRADEOS_DIR%\%ENV_FILE%'"

REM Update XTS market data credentials if applicable
if "%IS_XTS%"=="1" (
    powershell -Command "(Get-Content '%TRADEOS_DIR%\%ENV_FILE%') -replace 'BROKER_API_KEY_MARKET = ''YOUR_BROKER_MARKET_API_KEY''', 'BROKER_API_KEY_MARKET = ''!BROKER_API_KEY_MARKET!''' | Set-Content '%TRADEOS_DIR%\%ENV_FILE%'"
    powershell -Command "(Get-Content '%TRADEOS_DIR%\%ENV_FILE%') -replace 'BROKER_API_SECRET_MARKET = ''YOUR_BROKER_MARKET_API_SECRET''', 'BROKER_API_SECRET_MARKET = ''!BROKER_API_SECRET_MARKET!''' | Set-Content '%TRADEOS_DIR%\%ENV_FILE%'"
)
...
echo   Data directory: %TRADEOS_DIR%
echo   Config file:    %TRADEOS_DIR%\%ENV_FILE%
echo   Database:       %TRADEOS_DIR%\db\
echo   Strategies:     %TRADEOS_DIR%\strategies\
echo   Logs:           %TRADEOS_DIR%\log\
echo.
echo   Redirect URL for broker portal:
echo   http://127.0.0.1:5000/%BROKER_NAME%/callback
echo.
echo   Documentation: https://docs.tradeos.in
echo.
set /p OPEN_ENV="Open .env in Notepad for review? (y/n): "
if /i "%OPEN_ENV%"=="y" (
    start notepad "%TRADEOS_DIR%\%ENV_FILE%"
)
echo.
echo [OK] Setup complete! Run 'docker-run.bat start' to launch TradeOS.

:setup_end
exit /b %SETUP_FAILED%

:start
echo [INFO] Starting TradeOS...
echo.
...
if not exist "%TRADEOS_DIR%\%ENV_FILE%" (
    ...
    if errorlevel 1 (
        echo.
        echo [ERROR] Setup failed. Cannot start TradeOS.
        echo Please fix the issues above and try again.
        goto end
    )
    echo.
    echo [INFO] Starting TradeOS after setup...
    echo.
)
...
if not exist "%TRADEOS_DIR%\strategies\" (
    echo [INFO] Creating strategies directory...
    md "%TRADEOS_DIR%\strategies" 2>nul
    md "%TRADEOS_DIR%\strategies\scripts" 2>nul
    md "%TRADEOS_DIR%\strategies\examples" 2>nul
)
if not exist "%TRADEOS_DIR%\log\" (
    echo [INFO] Creating log directory...
    md "%TRADEOS_DIR%\log" 2>nul
    md "%TRADEOS_DIR%\log\strategies" 2>nul
)
if not exist "%TRADEOS_DIR%\keys\" (
    echo [INFO] Creating keys directory...
    md "%TRADEOS_DIR%\keys" 2>nul
)
if not exist "%TRADEOS_DIR%\tmp\" (
    echo [INFO] Creating temp directory...
    md "%TRADEOS_DIR%\tmp" 2>nul
)
...
REM See: https://github.com/marketcalls/tradeos/issues/822
...
docker run -d ^
    ...
    -e "TZ=Asia/Kolkata" ^
    -v "%TRADEOS_DIR%\db:/app/db" ^
    -v "%TRADEOS_DIR%\strategies:/app/strategies" ^
    -v "%TRADEOS_DIR%\log:/app/log" ^
    -v "%TRADEOS_DIR%\keys:/app/keys" ^
    -v "%TRADEOS_DIR%\tmp:/app/tmp" ^
    -v "%TRADEOS_DIR%\.env:/app/.env:ro" ^
...
if errorlevel 1 (
    ...
    echo   3. Check .env file: %TRADEOS_DIR%\%ENV_FILE%
    echo.
    goto end
)

echo.
echo [SUCCESS] TradeOS started successfully!
echo.
echo   ========================================
...
echo     docker-run.bat stop     - Stop TradeOS
echo     docker-run.bat restart  - Restart TradeOS
echo.
goto end

:stop
echo [INFO] Stopping TradeOS...
docker stop %CONTAINER% >nul 2>&1
docker rm %CONTAINER% >nul 2>&1
echo [OK] TradeOS stopped.
goto end

:restart
echo [INFO] Restarting TradeOS...
docker stop %CONTAINER% >nul 2>&1
docker rm %CONTAINER% >nul 2>&1
echo [OK] TradeOS stopped.
echo.
...
if errorlevel 1 (
    echo [STATUS] TradeOS is NOT running.
) else (
    echo [STATUS] TradeOS is running.
    echo.
    echo   Web UI: http://127.0.0.1:5000
)
echo.
...
echo.
echo Commands:
echo   start    Start TradeOS (runs setup if needed, default)
...
echo      curl.exe -O https://raw.githubusercontent.com/marketcalls/tradeos/main/install/docker-run.bat
echo   3. Run: docker-run.bat
echo.
echo Data Location: %TRADEOS_DIR%
echo   - Config:     %TRADEOS_DIR%\.env
echo   - Database:   %TRADEOS_DIR%\db\
echo   - Strategies: %TRADEOS_DIR%\strategies\
echo   - Logs:       %TRADEOS_DIR%\log\
echo.
