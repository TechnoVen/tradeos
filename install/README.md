...
# Or using curl
curl -O https://raw.githubusercontent.com/marketcalls/tradeos/main/install/install.sh
...
- Set up the TradeOS application with unique deployment name
...
- Each deployment gets a unique service name (e.g., tradeos-yourdomain-broker)
...
1. **Check Service Status**
   ...
   sudo systemctl status tradeos-fyers-yourdomain-fyers
   
   # Example for Zerodha deployment
   sudo systemctl status tradeos-zerodha-yourdomain-zerodha
   ```
...
2. **Application Not Starting**
   ...
   sudo journalctl -u tradeos-fyers-yourdomain-fyers    # View logs
   sudo systemctl restart tradeos-fyers-yourdomain-fyers # Restart service
   
   # Example 2: Zerodha deployment on zerodha.yourdomain.com
   sudo journalctl -u tradeos-zerodha-yourdomain-zerodha # View logs
   sudo systemctl restart tradeos-zerodha-yourdomain-zerodha # Restart service
   ```
...
1. **SELinux blocking Nginx**
   ...
   sudo semanage fcontext -a -t httpd_sys_rw_content_t "/var/python/tradeos-flask(/.*)?"
   sudo restorecon -Rv /var/python/tradeos-flask
   ```
...
1. **Service Management Examples**
   ...
   # Example outputs:
   # tradeos-fyers-yourdomain-fyers.service    loaded active running
   # tradeos-zerodha-yourdomain-zerodha.service loaded active running
   
   # Restart specific deployment
   sudo systemctl restart tradeos-fyers-yourdomain-fyers
...
2. **Log Management Examples**
   ...
   sudo journalctl -f -u tradeos-fyers-yourdomain-fyers
   
   # View last 100 lines of Zerodha deployment logs
   sudo journalctl -n 100 -u tradeos-zerodha-yourdomain-zerodha
   
   # View logs since last hour for specific deployment
   sudo journalctl --since "1 hour ago" -u tradeos-fyers-yourdomain-fyers
   ```
...
4. **Installation Directory Examples**
   ...
   ls -l /var/python/tradeos-flask/
   
   # Example structure:
   # /var/python/tradeos-flask/fyers-yourdomain-fyers/
   # /var/python/tradeos-flask/zerodha-yourdomain-zerodha/
   
   # Check specific deployment files
   ls -l /var/python/tradeos-flask/fyers-yourdomain-fyers/
   ```
...
2. **SELinux (RHEL-based systems)**
   - The installation script **automatically configures SELinux** for TradeOS
...
- Check the [GitHub repository](https://github.com/TechnoVen/tradeos)
