...
TELEGRAM_KEY_SALT = os.getenv("TELEGRAM_KEY_SALT", "telegram-tradeos-salt").encode()
...
class TelegramUser(Base):
    ...
    tradeos_username = Column(String(255), nullable=False, index=True)
    encrypted_api_key = Column(Text)  # Encrypted API key for secure storage
    host_url = Column(String(500))  # TradeOS host URL
    ...
    try:
        ...
        if user:
            result = {
                ...
                "tradeos_username": user.tradeos_username,
...
def get_telegram_user_by_username(username: str) -> dict | None:
    """Get telegram user by TradeOS username (cached for 30 minutes)"""
    ...
    try:
        user = (
            db_session.query(TelegramUser)
            .filter_by(tradeos_username=username, is_active=True)
            .first()
        )
...
def create_or_update_telegram_user(
    ...
    try:
        ...
        if user:
            ...
            user.tradeos_username = username
            if encrypted_key:
...
def delete_telegram_user(telegram_id: int) -> bool:
    ...
    try:
        ...
        if user:
            username = user.tradeos_username
