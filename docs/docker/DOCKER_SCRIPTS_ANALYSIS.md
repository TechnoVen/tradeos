...
2. Add `-v "$TRADEOS_DIR/keys:/app/keys"` volume
3. Add `-v "$TRADEOS_DIR/tmp:/app/tmp"` volume
...
1. Add `--shm-size=2g ^` after `--name %CONTAINER% ^`
2. Add `-v "%TRADEOS_DIR%\keys:/app/keys" ^` volume
3. Add `-v "%TRADEOS_DIR%\tmp:/app/tmp" ^` volume
4. Update setup function to create `keys` and `tmp` directories
...
- ❌ `tradeos_tmp` volume and mount - Required for numba cache
...
2. Add `tradeos_tmp:/app/tmp` volume mount
3. Add `tradeos_tmp:` volume definition
4. Remove duplicate `tradeos_logs` volume (unused)
...
- [ ] Test numba: `docker exec tradeos python -c "import numba; print('OK')"`
- [ ] Check shared memory: `docker inspect tradeos --format='{{.HostConfig.ShmSize}}'`
