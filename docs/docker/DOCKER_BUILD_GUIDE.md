# TradeOS Docker Build Guide

Complete guide to building and deploying TradeOS with numba/llvmlite/scipy support.
...
# Verify it's working
docker-compose exec tradeos python -c "import numba; import llvmlite; import scipy; print('✓ Success')"
```
...
```bash
# Build the image
docker build --no-cache -t tradeos:latest .
...
docker run -d \
  ...
  -p 8765:8765 \
  -v tradeos_db:/app/db \
  -v tradeos_log:/app/log \
  -v tradeos_strategies:/app/strategies \
  -v tradeos_keys:/app/keys \
...
```bash
docker ps | grep tradeos
```

Expected output:
```
CONTAINER ID   IMAGE             COMMAND          CREATED          STATUS          PORTS                                            NAMES
abc123def456   tradeos:latest   "/app/start.sh"  10 seconds ago   Up 9 seconds    0.0.0.0:5000->5000/tcp, 0.0.0.0:8765->8765/tcp   tradeos-web
```
...
**Test imports:**
```bash
docker-compose exec tradeos python -c "import numba; import llvmlite; import scipy; print('✓ Imports successful')"
```

**Test numba JIT:**
```bash
docker-compose exec tradeos python -c "
...
docker-compose exec tradeos ps aux | grep websocket_proxy
```
...
```bash
docker-compose exec tradeos ls -la /app/.env
docker-compose exec tradeos head -5 /app/.env
```
...
```bash
docker-compose exec tradeos ls -la /app/
docker-compose exec tradeos ls -la /app/tmp/

# Fix if needed (run as root)
docker-compose exec -u root tradeos chown -R appuser:appuser /app/tmp
docker-compose exec -u root tradeos chmod -R 755 /app/tmp
```
...
```bash
docker-compose exec tradeos bash -c '
...
docker buildx build --platform linux/arm64 -t tradeos:arm64 .

# For AMD64 (Intel/AMD)
docker buildx build --platform linux/amd64 -t tradeos:amd64 .

# Multi-platform (requires buildx)
docker buildx build --platform linux/amd64,linux/arm64 -t tradeos:latest .
```
...
```bash
docker-compose build --no-cache
docker tag tradeos:latest tradeos:v2.0.0
docker tag tradeos:latest myregistry.com/tradeos:latest
```
...
# Tag for registry
docker tag tradeos:latest your-registry.com/tradeos:latest

# Push
docker push your-registry.com/tradeos:latest
```
...
```bash
# Build
docker build -t tradeos:latest .

# Push to registry
docker tag tradeos:latest your-registry/tradeos:latest
docker push your-registry/tradeos:latest
...
3. **Scan image for vulnerabilities:**
   ...
   docker run --rm -v /var/run/docker.sock:/var/run/docker.sock \
     aquasec/trivy:latest image tradeos:latest
   ```
...
4. Pull the built image:
   ```bash
   docker pull marketcalls/tradeos:latest
   ```
...
- **TradeOS Documentation:** https://docs.tradeos.io
...
3. Check GitHub Issues: https://github.com/TechnoVen/tradeos/issues
...
# Shell access
docker-compose exec tradeos bash

# Run Python script
docker-compose exec tradeos uv run python /app/strategies/scripts/your_script.py

# Test dependencies
docker-compose exec tradeos python -c "import numba; import scipy; print('OK')"
