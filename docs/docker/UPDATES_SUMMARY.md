...
docker run -d \
    ...
    -v "$TRADEOS_DIR/strategies:/app/strategies" \
    -v "$TRADEOS_DIR/log:/app/log" \
    -v "$TRADEOS_DIR/keys:/app/keys" \                # ← NEW
    -v "$TRADEOS_DIR/tmp:/app/tmp" \                  # ← NEW
    -v "$TRADEOS_DIR/.env:/app/.env:ro" \
    --restart unless-stopped \
    "$IMAGE"
```
...
services:
  tradeos:
    ...
    volumes:
      - tradeos_db:/app/db
      - tradeos_logs:/app/logs          # ← EXTRA, UNUSED
      - tradeos_log:/app/log
...
- [ ] Verify numba import works: `docker exec tradeos python -c "import numba; print('OK')"`
- [ ] Verify shared memory: `docker inspect tradeos --format='{{.HostConfig.ShmSize}}'` should show `2147483648`
...
```bash
docker inspect tradeos --format='{{.HostConfig.ShmSize}}'
# Should show: 2147483648 (2GB in bytes)
```

**Verify volumes:**
```bash
docker inspect tradeos --format='{{range .Mounts}}{{.Destination}} {{end}}'
