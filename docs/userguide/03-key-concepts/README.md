...
Before diving into TradeOS, let's understand the key terms and concepts you'll encounter. This foundation will make everything else easier to understand.
...
**In TradeOS**: When TradingView wants to place an order, it sends a request to TradeOS's API. TradeOS processes it and sends the order to your broker.

```
TradingView → "Place BUY order for SBIN" → TradeOS API → Broker
```

### 2. API Key

**Simple Explanation**: Your API key is like a password that identifies you. It proves to TradeOS that the request is coming from an authorized source.
...
**Simple Explanation**: A webhook is like a doorbell. When something happens (like a TradingView alert), it "rings" your TradeOS server to notify it.
...
Webhook sends data to your URL
        ↓
TradeOS receives and processes
        ↓
...
**Simple Explanation**: When you log into your broker through TradeOS, you get a temporary pass (token) that lets TradeOS place orders on your behalf.
...
**Simple Explanation**: Every stock has a specific way to write its name that TradeOS understands.

**Examples**:
| What you want | TradeOS Symbol |
|---------------|-----------------|
...
**Simple Explanation**: The period you're logged into TradeOS. Sessions expire for security.

**Browser Session**: Your TradeOS web login
...
**Previous**: [02 - Why Build with TradeOS](../02-why-build-with-tradeos/README.md)
