...
This guide walks you through installing TradeOS on your system. We'll cover Windows, Ubuntu, and macOS installations.
...
#### Step 3: Download TradeOS

Open Command Prompt (search "cmd") and run:

```cmd
# Navigate to where you want TradeOS
...
#### Step 3: Download and Run TradeOS
...
You should see the TradeOS login page.
...
For production use, deploy TradeOS on an Ubuntu server using the automated `install.sh` script. This is the **recommended approach** for live trading.
...
```bash
mkdir -p ~/tradeos.iostall
cd ~/tradeos.iostall

wget https://raw.githubusercontent.com/marketcalls/tradeos/main/install/install.sh
...
- Unique service name (e.g., tradeos-yourdomain-broker)
...
1. **Check Service Status**
   ```bash
   sudo systemctl status tradeos-yourdomain-broker
   ```
...
```bash
# List all TradeOS services
systemctl list-units "tradeos-*"

# Restart specific deployment
sudo systemctl restart tradeos-yourdomain-broker

# View real-time logs
sudo journalctl -f -u tradeos-yourdomain-broker

# View last 100 lines of logs
sudo journalctl -n 100 -u tradeos-yourdomain-broker
```
...
```bash
# View service logs
sudo journalctl -u tradeos-yourdomain-broker
...
**Important**: Tunneling is **only for webhooks**. Always run TradeOS on your own server with proper domain setup for production use. Don't run the entire application through a tunnel.
...
TradeOS can also be deployed using Docker with custom domain and SSL. This is useful if you prefer containerized deployments.
...
5. Clones TradeOS to `/opt/tradeos`
...
TradeOS can run on Raspberry Pi models 3, 4, or 5 (4GB+ RAM), preferably with Ubuntu 24.04+ server edition.
...
**Clone and Build:**
```bash
git clone https://github.com/TechnoVen/tradeos
cd tradeos
cp .sample.env .env
...
Close all TradeOS instances and restart.
