...
ChartInk is a powerful stock screening platform popular among Indian traders. It can scan thousands of stocks in real-time and send webhook alerts when conditions are met. TradeOS integrates with ChartInk to execute trades automatically based on your screener alerts.

## How It Works

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    ChartInk → TradeOS Flow                                 │
│                                                                              │
│  ┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌──────────┐  │
│  │  ChartInk   │     │   Webhook   │     │  TradeOS   │     │  Broker  │  │
...
2. TradeOS running and accessible via internet
3. API key generated in TradeOS
4. Broker connected and logged in

## Making TradeOS Accessible for Webhooks

ChartInk webhooks need to reach your TradeOS server from the internet.

### Recommended: Production Server with Domain

Deploy TradeOS on an Ubuntu server using `install.sh` (see [Installation Guide](../04-installation/README.md)):
...
**Important**: Tunnel services are **only for webhooks**, not for running the full application. Always run TradeOS on your own server for production use
...
2. Enable **Webhook**
3. Enter your TradeOS webhook URL
...
```
https://your-tradeos-url/api/v1/placeorder
```

### ChartInk Webhook Payload

ChartInk sends data in a specific format. You need to configure the payload to match TradeOS's API:
...
1. Enable Analyzer Mode in TradeOS
...
- Check TradeOS dashboard regularly
- Set up Telegram notifications
- Review trades daily

## Troubleshooting

### Alert Not Reaching TradeOS
...
1. Go to **Traffic Logs** in TradeOS
