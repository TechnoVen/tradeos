...
Amibroker is a powerful technical analysis and backtesting software widely used by Indian traders. TradeOS provides seamless integration via its HTTP API, allowing your Amibroker strategies to execute trades automatically.

## How It Works

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Amibroker → TradeOS Flow                                │
│                                                                              │
│  ┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌──────────┐  │
│  │  Amibroker  │     │    HTTP     │     │  TradeOS   │     │  Broker  │  │
...
2. TradeOS running on same machine or network
...
```afl
function SendTradeOSOrder(apiKey, strategy, symbol, exchange, action, quantity, priceType, product)
{
    // Build JSON payload
    json = "{";
    json += "\"apikey\": \"" + apiKey + "\",";
...
{
    response = SendTradeOSOrder(apiKey, strategy, symbol, exchange, "BUY", quantity, priceType, product);
    _TRACE("Buy Order: " + response);
}

if(LastValue(Sell) AND LastValue(Ref(Sell, -1)) == 0)
{
    response = SendTradeOSOrder(apiKey, strategy, symbol, exchange, "SELL", quantity, priceType, product);
...
```afl
//=============================================================================
// TradeOS Integration Template for Amibroker
// Version: 1.0
//=============================================================================

// Configuration Section
_SECTION_BEGIN("TradeOS Configuration");

apiKey = ParamStr("API Key", "YOUR_API_KEY");
baseUrl = ParamStr("TradeOS URL", "http://127.0.0.1:5000");
...
Always test with Analyzer Mode enabled in TradeOS first.
...
| Issue | Solution |
|-------|----------|
| Connection refused | Check TradeOS is running |
| Invalid API key | Verify key in TradeOS |
