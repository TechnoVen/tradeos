...
The Flow Visual Strategy Builder is TradeOS's node-based visual programming interface. It allows you to create trading strategies without writing code by connecting nodes in a flowchart-like canvas.
...
**Recommended**: Deploy TradeOS on an Ubuntu server with your domain using `install.sh`:
```
...
```
https://your-tradeos-url/api/v1/flow/{flow-id}/webhook
```
...
TradeOS provides pre-built templates:
