...
Guidelines for upgrading TradeOS to new versions while preserving data and configurations.
...
│  □ 5. Stop running TradeOS instance                                       │
...
cp db/tradeos.db backups/$(date +%Y%m%d)/
...
TradeOS uses automatic database initialization on startup. New tables are created automatically when the application starts - no manual migrations are required.
...
# Restore databases
cp backups/YYYYMMDD/tradeos.db db/
