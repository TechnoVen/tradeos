...
TradeOS implements centralized logging with configurable levels, file rotation, and structured output. All application logs are routed through a unified logging system stored in `logs.db` and optional file logs.
    ...
    if not logger.handlers:
        ...
        if os.getenv('LOG_TO_FILE', 'False').lower() == 'true':
            file_handler = RotatingFileHandler(
                filename=os.path.join(os.getenv('LOG_DIR', 'log'), 'tradeos.log'),
...
```
log/
├── tradeos.log        # Current log file
├── tradeos.log.1      # Previous rotation
├── tradeos.log.2      # Older rotation
├── ...
└── tradeos.log.14     # Oldest (based on LOG_RETENTION)
```
