...
The PlaceOrder API is the core order execution endpoint in TradeOS. It handles order validation, authentication, broker routing, and response processing through multiple layers.
...
| Invalid API key | 403 | `{"status": "error", "message": "Invalid tradeos apikey"}` |
