...
TradeOS implements a two-phase authentication system:
1. **User Authentication** - Username/password login to TradeOS
2. **Broker Authentication** - OAuth2/TOTP/API-based login to trading broker

This design ensures users first authenticate with TradeOS before connecting to their broker account.
...
TradeOS supports 24+ brokers with different authentication methods:
