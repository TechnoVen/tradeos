...
Each broker in TradeOS follows a standardized folder structure with consistent interfaces for authentication, order management, data retrieval, and symbol mapping.
...
Convert TradeOS format to broker format.
...
def transform_response(response):
    """Transform broker response to TradeOS format"""
    return {
...
def get_symbol(symbol, exchange):
    """Get broker symbol from TradeOS symbol"""
    pass
