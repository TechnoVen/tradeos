#!/bin/bash
# TradeOS Docker Build and Deployment Script
# This script builds and deploys TradeOS with numba/llvmlite support
...
# Start container
start_container() {
    print_header "Starting TradeOS Container"
...
health_check() {
    ...
    if docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
        ...
        docker-compose logs --tail=50 tradeos || docker logs ${CONTAINER_NAME}
        exit 1
    fi
...
test_python_deps() {
    ...
    if docker-compose exec -T tradeos python -c "import numba; import llvmlite; import scipy; print('SUCCESS')" 2>/dev/null | grep -q "SUCCESS"; then
    ...
    if docker-compose exec -T tradeos python -c "
...
" 2>/dev/null | grep -q "SUCCESS"; then
    ...
    if docker-compose exec -T tradeos bash -c "
...
show_access_info() {
    ...
    echo -e "${GREEN}✓ TradeOS is now running${NC}\n"
    ...
    echo -e "  Shell access:     ${YELLOW}docker-compose exec tradeos bash${NC}"
    echo -e "  Run strategy:     ${YELLOW}docker-compose exec tradeos uv run python /app/strategies/scripts/your_script.py${NC}"
...
# Main execution
main() {
    print_header "TradeOS Docker Build & Deploy"
