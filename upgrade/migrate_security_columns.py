...
def migrate_settings_table():
    ...
    DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///db/tradeos.db")

    # Adjust path for SQLite if relative (since we're in upgrade folder)
    if DATABASE_URL.startswith("sqlite:///") and not DATABASE_URL.startswith("sqlite:////"):
        # Extract the relative path
        db_path = DATABASE_URL.replace("sqlite:///", "")
        # Make it relative to parent directory (tradeos root)
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("TradeOS Security Columns Migration Script")
    logger.info("=" * 60)
    ...
    if success:
        ...
        logger.info("\n📌 Next Steps:")
        logger.info("   1. Restart your TradeOS application")
