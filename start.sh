...
if [ -f "$ENV_FILE" ] && [ -r "$ENV_FILE" ] && [ -s "$ENV_FILE" ]; then
    echo "[TradeOS] Using existing .env file"
else
    echo "[TradeOS] No .env file found or file is empty. Checking for environment variables..."
    
    # Check if we're on Railway/Cloud (HOST_SERVER is the key indicator)
    if [ -n "$HOST_SERVER" ]; then
        echo "[TradeOS] Environment variables detected. Generating .env file..."
        ...
        if ! touch "$ENV_FILE" 2>/dev/null; then
            echo "[TradeOS] Cannot write to /app/.env, using /tmp/.env"
            ENV_FILE="/tmp/.env"
        fi
        ...
        cat > "$ENV_FILE" << EOF
# TradeOS Environment Configuration File
...
DATABASE_URL = '${DATABASE_URL:-sqlite:///db/tradeos.db}'
...
EOF
        ...
        echo "[TradeOS] .env file generated at $ENV_FILE"
        echo "[TradeOS] Configuration: HOST_SERVER=${HOST_SERVER}"
        ...
        if [ "$ENV_FILE" = "/tmp/.env" ]; then
            ...
            echo "[TradeOS] Linked .env to /app/.env"
        fi
    else
        echo "============================================"
...
if [ -f "/app/upgrade/migrate_all.py" ]; then
    echo "[TradeOS] Running database migrations..."
    /app/.venv/bin/python /app/upgrade/migrate_all.py || echo "[TradeOS] Migration completed (some may have been skipped)"
else
    echo "[TradeOS] No migrations found, skipping..."
fi

# ============================================
# WEBSOCKET PROXY SERVER
# ============================================
echo "[TradeOS] Starting WebSocket proxy server on port 8765..."
/app/.venv/bin/python -m websocket_proxy.server &
WEBSOCKET_PID=$!
echo "[TradeOS] WebSocket proxy server started with PID $WEBSOCKET_PID"

# ============================================
# CLEANUP HANDLER
# ============================================
cleanup() {
    echo "[TradeOS] Shutting down..."
...
echo "[TradeOS] Starting application on port ${APP_PORT} with eventlet..."
