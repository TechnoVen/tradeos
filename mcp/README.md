...
This is a Model Context Protocol (MCP) server that provides trading and market data functionality through the TradeOS platform. It enables AI assistants to execute trades, manage positions, and retrieve market data directly from supported brokers.

## Prerequisites

### 1. TradeOS Server Setup

Ensure your TradeOS server is running and properly configured:

1. **Start TradeOS Server**: Your TradeOS server should be running (e.g., on `http://127.0.0.1:5000`)
2. **Verify Connection**: Test that the server is accessible by visiting the web interface.
3. **Broker Authentication**: Ensure your broker credentials are properly configured in TradeOS.

### 2. API Key

To get your TradeOS API key:
1. Open your TradeOS web interface (e.g., `http://127.0.0.1:5000`)
...
```json
{
  "mcpServers": {
    "tradeos": {
      "command": "D:\\tradeos-mcp\\tradeos\\.venv\\Scripts\\python.exe",
      "args": [
        "D:\\tradeos-mcp\\tradeos\\mcp\\mcpserver.py",
        "YOUR_API_KEY_HERE",
        "http://127.0.0.1:5000"
      ]
    }
...
```json
{
  "mcpServers": {
    "tradeos": {
      "command": "/Users/your_username/tradeos/.venv/bin/python3",
      "args": [
        "/Users/your_username/tradeos/mcp/mcpserver.py",
        "YOUR_API_KEY_HERE",
        "http://127.0.0.1:5000"
      ]
    }
...
```json
{
  "mcpServers": {
    "tradeos": {
      "command": "/home/your_username/tradeos/.venv/bin/python3",
      "args": [
        "/home/your_username/tradeos/mcp/mcpserver.py",
        "YOUR_API_KEY_HERE",
        "http://127.0.0.1:5000"
      ]
    }
...
- **Windows**: Replace `D:\\tradeos-zerodha\\tradeos` with your actual TradeOS installation path
...
### Utilities
- `get_tradeos_version` - Check TradeOS version
...
1. **Connection Issues**: Verify TradeOS server is running on `http://127.0.0.1:5000`
...
4. **Order Failures**: Verify broker credentials in TradeOS are valid and active

## Support

For issues related to:
- **TradeOS Platform**: Visit the TradeOS documentation
