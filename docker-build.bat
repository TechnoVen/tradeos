@echo off
REM TradeOS Docker Build and Deployment Script for Windows
REM This script builds and deploys TradeOS with numba/llvmlite support

setlocal enabledelayedexpansion

set IMAGE_NAME=tradeos
set IMAGE_TAG=latest
set CONTAINER_NAME=tradeos-web

echo.
echo ========================================
echo TradeOS Docker Build ^& Deploy
echo ========================================
echo.
...
REM Start container
echo.
echo [5/8] Starting TradeOS container...
docker-compose up -d
if %ERRORLEVEL% NEQ 0 (
...
docker-compose exec -T tradeos python -c "import numba; import llvmlite; import scipy; print('SUCCESS')" 2>nul | findstr "SUCCESS" >nul
if %ERRORLEVEL% EQU 0 (
...
docker-compose exec -T tradeos python -c "from numba import jit; import numpy as np; jit(nopython=True)(lambda x: x*2)(np.array([1,2,3])); print('SUCCESS')" 2>nul | findstr "SUCCESS" >nul
if %ERRORLEVEL% EQU 0 (
...
docker-compose exec -T tradeos python -c "from scipy import stats; print('SUCCESS' if abs(stats.norm.cdf(0) - 0.5) ^< 0.001 else 'FAILED')" 2>nul | findstr "SUCCESS" >nul
if %ERRORLEVEL% EQU 0 (
...
echo   Shell access:     docker-compose exec tradeos bash
echo.
