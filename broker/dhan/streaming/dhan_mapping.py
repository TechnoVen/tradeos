...
class DhanExchangeMapper:
    """Maps between TradeOS exchange names and Dhan exchange codes"""

    # TradeOS to Dhan exchange mapping
    EXCHANGE_MAP = {
        "NSE": "NSE_EQ",
        "BSE": "BSE_EQ",
        "NFO": "NSE_FNO",
    ...
    # Dhan exchange segment codes (numeric) to TradeOS exchange mapping
    ...
    # Reverse mappings
    DHAN_TO_TRADEOS = {v: k for k, v in EXCHANGE_MAP.items()}
    EXCHANGE_TO_SEGMENT = {v: k for k, v in SEGMENT_TO_EXCHANGE.items()}

    @classmethod
    def get_dhan_exchange(cls, tradeos_exchange: str) -> str | None:
        """Convert TradeOS exchange to Dhan exchange format"""
        return cls.EXCHANGE_MAP.get(tradeos_exchange)

    @classmethod
    def get_tradeos_exchange(cls, dhan_exchange: str) -> str | None:
        """Convert Dhan exchange to TradeOS exchange format"""
        return cls.DHAN_TO_TRADEOS.get(dhan_exchange)

    @classmethod
    def get_exchange_from_segment(cls, segment_code: int) -> str | None:
        """Convert Dhan exchange segment code to TradeOS exchange"""
    ...
    def get_segment_from_exchange(cls, exchange: str) -> int | None:
        """Convert TradeOS exchange to Dhan exchange segment code"""
