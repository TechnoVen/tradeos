...
def transform_margin_position(position, client_id):
    """
    Transform a single TradeOS margin position to Dhan margin format.
...
def map_product_type_for_margin(product):
    """
    Maps TradeOS product type to Dhan product type for margin calculation.
...
def parse_margin_response(response_data):
    """
    Parse Dhan margin response to TradeOS standard format.
...
def parse_batch_margin_response(responses):
    ...
    This is a limitation of the Dhan API, not TradeOS.

    Args:
        responses: List of individual margin responses (one per leg)

    Returns:
        Aggregated margin response matching TradeOS format
    """
    try:
        total_margin = 0
        total_span = 0
