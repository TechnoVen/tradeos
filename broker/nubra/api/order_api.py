...
def get_api_response(endpoint, auth, method="GET", payload=""):
    AUTH_TOKEN = auth
    device_id = "TRADEOS"  # Fixed device ID, same as auth_api.py
...
def place_order_api(data, auth):
    ...
    # Transform TradeOS data to Nubra format
    ...
    if response.status_code in [200, 201] and "order_id" in response_data:
        ...
        # Normalize response format for TradeOS compatibility
        response_data["status"] = True
        response_data["data"] = {"orderid": orderid}
        # TradeOS service layer expects status 200 for success
        response.status = 200
    else:
        orderid = None
...
def close_all_positions(current_api_key, auth):
    ...
    for position in positions:
        ...
        # Try to get TradeOS symbol from database using ref_id
...
def modify_order(data, auth):
    ...
    # Transform TradeOS data to Nubra modify order format
