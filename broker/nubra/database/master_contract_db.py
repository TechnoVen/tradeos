...
def process_nubra_json(path):
    ...
    and converts it into TradeOS master contract format.
    ...
    - Symbol format follows TradeOS F&O spec:
    ...
    # ---------------- Symbol Construction (TradeOS format) ----------------
...
def process_nubra_indexes(path):
    """
    Processes the Nubra index CSV file to fit the TradeOS database schema.
    ...
    Output format follows TradeOS convention:
    - exchange = NSE_INDEX or BSE_INDEX
    - brexchange = NSE or BSE (original exchange)
    - instrumenttype = 'INDEX'
    - symbol = Standardized TradeOS format (NIFTY, BANKNIFTY, SENSEX, etc.)
    ...
    # Map CSV columns to TradeOS database schema
    ...
    # Map to TradeOS index exchange format
