...
This document outlines a critical session management issue identified with the Flattrade WebSocket API and the corresponding architectural fix implemented in the TradeOS proxy.
...
5.  Restarting the main TradeOS application would temporarily resolve the problem, allowing the next connection to succeed.
