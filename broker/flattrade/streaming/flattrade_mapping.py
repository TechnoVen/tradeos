...
class FlattradeExchangeMapper:
    """Maps between TradeOS exchange names and Flattrade exchange codes"""

    # TradeOS to Flattrade exchange mapping
    EXCHANGE_MAP = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NFO": "NFO",
    ...
    # Reverse mapping
    FLATTRADE_TO_TRADEOS = {v: k for k, v in EXCHANGE_MAP.items()}

    @classmethod
    def to_flattrade_exchange(cls, oa_exchange: str) -> str | None:
        """Convert TradeOS exchange to Flattrade exchange format"""
        return cls.EXCHANGE_MAP.get(oa_exchange.upper())

    @classmethod
    def to_oa_exchange(cls, flattrade_exchange: str) -> str | None:
        """Convert Flattrade exchange to TradeOS exchange format"""
        return cls.FLATTRADE_TO_TRADEOS.get(flattrade_exchange.upper())
