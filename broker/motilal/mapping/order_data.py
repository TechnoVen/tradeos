...
def map_order_data(order_data):
    ...
    if order_data:
        for order in order_data:
            ...
            # Convert Motilal exchange (NSEFO) to TradeOS exchange (NFO) for database lookup
            tradeos_exchange = reverse_map_exchange(motilal_exchange)

            # Use the get_symbol function to fetch the symbol from the database
            # Use TradeOS exchange format for lookup
            symbol_from_db = get_symbol(symboltoken, tradeos_exchange)

            # Check if a symbol was found; if so, update the trading_symbol in the current order
            if symbol_from_db:
                order["symbol"] = symbol_from_db  # Motilal uses 'symbol' field
                # Convert exchange to TradeOS format for display
                order["exchange"] = tradeos_exchange

                # Map Motilal product types to TradeOS format
                if tradeos_exchange in ["NSE", "BSE"]:
                ...
                elif tradeos_exchange in ["NFO", "MCX", "CDS", "BFO"]:
                ...
                logger.info(
                    f"Symbol not found for token {symboltoken} and exchange {tradeos_exchange}. Keeping original trading symbol."
                )
                # Still convert exchange to TradeOS format
                order["exchange"] = tradeos_exchange
...
def transform_order_data(orders):
    ...
    for order in orders:
        ...
        # Map Motilal order types to TradeOS standard format
        # Motilal returns: Market, Limit, Stoploss (title case)
        # TradeOS standard: MARKET, LIMIT, SL, SL-M (uppercase)
        ...
        # Map Motilal order status to TradeOS standard format
        # Motilal returns: Traded, Confirm, Sent, Error, Rejected, Cancel (title case)
        # TradeOS standard: complete, open, rejected, cancelled (lowercase)
...
def map_trade_data(trade_data):
    ...
    if trade_data:
        for order in trade_data:
            ...
            motilal_exchange = order["exchange"]
            # Convert Motilal exchange to TradeOS exchange for database lookup
            tradeos_exchange = reverse_map_exchange(motilal_exchange)

            # Use the get_symbol function to fetch the symbol from the database
            symbol_from_db = get_oa_symbol(symbol, tradeos_exchange)

            # Check if a symbol was found; if so, update the trading_symbol in the current order
            if symbol_from_db:
                order["symbol"] = symbol_from_db
                # Convert exchange to TradeOS format
                order["exchange"] = tradeos_exchange
                ...
                logger.info(
                    f"Unable to find the symbol {symbol} and exchange {tradeos_exchange}. Keeping original trading symbol."
                )
                order["exchange"] = tradeos_exchange
...
def transform_tradebook_data(tradebook_data):
    """
    Transforms Motilal Oswal tradebook data to TradeOS format.
    ...
    if position_data_list:
        for position in position_data_list:
            ...
            if symbol_from_db:
                ...
                # Convert exchange to TradeOS format
                position["exchange"] = tradeos_exchange
                ...
                logger.info(
                    f"Symbol not found for token {symboltoken} and exchange {tradeos_exchange}. Keeping original symbol."
                )
                position["exchange"] = tradeos_exchange
...
def transform_positions_data(positions_data):
    """
    Transforms Motilal Oswal positions data to TradeOS format.
...
def transform_holdings_data(holdings_data):
    """
    Transforms Motilal Oswal holdings data to TradeOS format.
    ...
    for holdings in holdings_list:
        # Get the mapped TradeOS symbol and exchange from map_portfolio_data
...
def map_portfolio_data(portfolio_data):
    ...
    Returns:
    - The modified portfolio_data with mapped fields (TradeOS symbols and exchange).
    """
    ...
    if isinstance(data, list):
        for idx, holding in enumerate(data):
            ...
            # Use get_symbol to fetch the TradeOS symbol from database
