...
def process_zebu_nse_data(output_path):
    """
    Processes the Zebu NSE data (NSE_symbols.txt) to generate TradeOS symbols.
    ...
    def get_tradeos_symbol(broker_symbol):
        ...
        if "-EQ" in broker_symbol:
            ...
            # For other symbols (including index), TradeOS symbol remains the same as broker symbol
    ...
    # Map common NSE index symbols to TradeOS format
    nse_index_mapping = {
        "NIFTY INDEX": "NIFTY",
...
def process_zebu_nfo_data(output_path):
    """
    Processes the Zebu NFO data (NFO_symbols.txt) to generate TradeOS symbols.
...
def process_zebu_cds_data(output_path):
    """
    Processes the Zebu CDS data (CDS_symbols.txt) to generate TradeOS symbols.
    Handles both futures and options formatting.
    """
...
def process_zebu_mcx_data(output_path):
    """
    Processes the Zebu MCX data (MCX_symbols.txt) to generate TradeOS symbols.
    Handles both futures and options formatting.
    """
...
def process_zebu_bse_data(output_path):
    """
    Processes the Zebu BSE data (BSE_symbols.txt) to generate TradeOS symbols.
    ...
    # Apply transformation for TradeOS symbols (no special logic needed here)
    def get_tradeos_symbol(broker_symbol):
        return broker_symbol
...
def process_zebu_bfo_data(output_path):
    """
    Processes the Zebu BFO data (BFO_symbols.txt) to generate TradeOS symbols and correctly extract the name column.
