...
class ZebuExchangeMapper:
    """Maps between TradeOS exchange names and Zebu exchange codes"""

    # TradeOS to Zebu exchange mapping (same as Flattrade/Noren)
    EXCHANGE_MAP = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NFO": "NFO",
    ...
    # Reverse mapping
    ZEBU_TO_TRADEOS = {v: k for k, v in EXCHANGE_MAP.items()}

    @classmethod
    def to_zebu_exchange(cls, oa_exchange: str) -> str | None:
        """Convert TradeOS exchange to Zebu exchange format"""
        return cls.EXCHANGE_MAP.get(oa_exchange.upper())

    @classmethod
    def to_oa_exchange(cls, zebu_exchange: str) -> str | None:
        """Convert Zebu exchange to TradeOS exchange format"""
        return cls.ZEBU_TO_TRADEOS.get(zebu_exchange.upper())
