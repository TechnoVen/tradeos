"""
Fyers Data Mapping
Maps Fyers HSM data to TradeOS format for compatibility
"""
...
class FyersDataMapper:
    """
    Maps Fyers HSM WebSocket data to TradeOS format
    """
    ...
    def map_to_tradeos_ltp(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers data to TradeOS LTP format

        Args:
            fyers_data: Raw data from Fyers HSM WebSocket

        Returns:
            TradeOS LTP format dict or None if mapping fails
        """
        try:
    ...
    def map_to_tradeos_quote(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers data to TradeOS Quote format

        Args:
            fyers_data: Raw data from Fyers HSM WebSocket

        Returns:
            TradeOS Quote format dict or None if mapping fails
        """
        try:
            if not fyers_data:
                return None
    ...
    def map_to_tradeos_depth(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers depth data to TradeOS Depth format

        Args:
            fyers_data: Raw depth data from Fyers HSM WebSocket

        Returns:
            TradeOS Depth format dict or None if mapping fails
        """
        try:
            ...
            # Map to TradeOS Depth format (matching other brokers)
            tradeos_data = {
                "symbol": f"{exchange}:{symbol_name}",
                "exchange": exchange,
                "token": fyers_data.get("exchange_token", ""),
    ...
    def map_index_to_synthetic_depth(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers index data to synthetic TradeOS Depth format
    ...
    def map_fyers_data(
        ...
        """
        Map Fyers data to appropriate TradeOS format based on requested type
        ...
        Returns:
            Mapped TradeOS data or None if mapping fails
        """
        if not fyers_data:
            return None
        ...
        if requested_type == "LTP":
            return self.map_to_tradeos_ltp(fyers_data)
        elif requested_type == "Quote":
            return self.map_to_tradeos_quote(fyers_data)
        elif requested_type == "Depth" and fyers_type == "dp":
            return self.map_to_tradeos_depth(fyers_data)
    ...
    def extract_symbol_info(self, symbol: str) -> dict[str, str]:
        """
        Extract exchange and symbol from TradeOS format
