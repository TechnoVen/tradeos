"""
Fyers WebSocket Adapter for TradeOS
...
class FyersAdapter:
    """
    Fyers WebSocket adapter for TradeOS streaming service
    Follows TradeOS adapter pattern similar to Angel, Zerodha etc.
    """
    ...
    def subscribe_symbols(self, symbols: list[dict[str, str]], data_type: str, callback: Callable):
        ...
        try:
            with self.lock:
                ...
                self.logger.debug(
                    f"Converting {len(valid_symbols)} TradeOS symbols to HSM format using database lookup..."
                )

                # Convert TradeOS symbols directly to HSM tokens using database lookup
                hsm_tokens, token_mappings, invalid_symbols = (
                    self.token_converter.convert_tradeos_symbols_to_hsm(valid_symbols, data_type)
                )
                ...
                # Since convert_tradeos_symbols_to_hsm processes symbols in order
    ...
    def _on_message(self, fyers_data: dict[str, Any]):
        ...
        try:
            ...
            # Map to TradeOS format first to get symbol info
            ...
            callback = None
            tradeos_data_type = "Quote"  # Default
