...
def map_broker_exchange_to_tradeos(broker_exchange, instrumenttype=""):
    """
    Maps mStock broker exchange to TradeOS exchange format.
    mStock returns NSE for both equity and derivatives,
    but TradeOS uses NFO for derivatives.
    ...
    Returns:
    - TradeOS exchange format (e.g., 'NFO' for NSE derivatives)
    """
...
def map_order_data(order_data):
    ...
    Converts broker symbols to TradeOS symbols and maps product types.
    ...
    if order_data:
        for order in order_data:
            ...
            # Get TradeOS symbol from database using symboltoken
            ...
            # Map product types to TradeOS format
...
def transform_order_data(orders):
    """
    Transforms mStock Type B order data to TradeOS format.

    Parameters:
    - orders: List of order dictionaries from mStock Type B API

    Returns:
    - List of orders in TradeOS format
    """
    ...
    for order in orders:
        ...
        # Normalize status to TradeOS format (lowercase)
        ...
        transformed_order = {
            ...
            "exchange": map_broker_exchange_to_tradeos(
...
def map_trade_data(trade_data):
    ...
    for trade in trade_data:
        ...
        if exchange:
            # First try: Get TradeOS symbol using symboltoken
            if symboltoken:
                symbol_from_db = get_symbol(symboltoken, lookup_exchange)
                if symbol_from_db:
...
def transform_tradebook_data(tradebook_data):
    """
    Transforms mStock Type B tradebook data to TradeOS format.

    Parameters:
    - tradebook_data: List of trade dictionaries from mStock Type B API

    Returns:
    - List of trades in TradeOS format
    """
    transformed_data = []
...
def transform_positions_data(positions_data):
    """
    Transforms mStock Type B positions data to TradeOS format.

    Parameters:
    - positions_data: List of position dictionaries (already mapped by map_position_data)

    Returns:
    - List of positions in TradeOS format
    """
    transformed_data = []
...
def transform_holdings_data(holdings_data):
    """
    Transforms mStock Type B holdings data to TradeOS format.
    ...
    Returns:
    - List of holdings in TradeOS format
    """
    transformed_data = []
...
def map_portfolio_data(portfolio_data):
    ...
    # Convert to expected format for compatibility with TradeOS
    if isinstance(data, list):
        ...
        for holding in data:
            ...
            # Get TradeOS symbol from broker symbol
            if symbol:
