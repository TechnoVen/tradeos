...
class ZerodhaExchangeMapper:
    """Maps exchange codes between Zerodha and TradeOS formats"""

    # Map TradeOS exchange codes to Zerodha exchange codes
    _OA_TO_ZERODHA = {
        "NSE": "NSE",
        "NFO": "NFO",
        "CDS": "CDS",
    ...
    # Map Zerodha exchange codes to TradeOS exchange codes
    _ZERODHA_TO_OA = {v: k for k, v in _OA_TO_ZERODHA.items()}

    @classmethod
    def to_zerodha_exchange(cls, oa_exchange: str) -> str:
        """
        Convert TradeOS exchange code to Zerodha exchange code.

        Args:
            oa_exchange: TradeOS exchange code (e.g., 'NSE', 'NFO')
    ...
    def to_oa_exchange(cls, zerodha_exchange: str) -> str:
        """
        Convert Zerodha exchange code to TradeOS exchange code.

        Args:
            zerodha_exchange: Zerodha exchange code

        Returns:
            TradeOS exchange code
        """
...
class ZerodhaCapabilityRegistry:
    ...
    # Map TradeOS capability flags to Zerodha subscription modes
    CAPABILITY_MAP = {
        "LTP": "ltp",
        "QUOTE": "quote",
        "DEPTH": "full",
    ...
    def get_zerodha_mode(cls, capability: str) -> str:
        ...
        Args:
            capability: TradeOS capability (LTP, QUOTE, DEPTH)
...
class ZerodhaDataTransformer:
    """Transforms data between Zerodha and TradeOS formats"""
    ...
    def transform_tick(self, tick_data: dict, symbol: str, exchange: str) -> dict:
        """
        Transform Zerodha tick data to TradeOS format.
        ...
        Returns:
            Transformed tick data in TradeOS format
        """
        try:
            if not tick_data:
                return {}
    ...
    def transform_order_update(self, order_data: dict) -> dict:
        """
        Transform Zerodha order update to TradeOS format.

        Args:
            order_data: Raw order data from Zerodha WebSocket

        Returns:
            Transformed order data in TradeOS format
        """
        try:
            ...
            # Map Zerodha status to TradeOS status
            status_map = {
                "OPEN": "open",
                "COMPLETE": "complete",
                "CANCELLED": "cancelled",
    ...
    def transform_position(self, position_data: dict) -> dict:
        """
        Transform Zerodha position data to TradeOS format.

        Args:
            position_data: Raw position data from Zerodha

        Returns:
            Transformed position data in TradeOS format
        """
        try:
