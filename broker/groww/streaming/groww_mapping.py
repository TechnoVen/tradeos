...
class GrowwExchangeMapper:
    """Maps TradeOS exchange codes to Groww exchange/segment format"""

    # Mapping from TradeOS exchange to Groww exchange and segment
    EXCHANGE_MAP = {
    ...
    @classmethod
    def get_exchange(cls, tradeos_exchange: str) -> str:
        """
        Get Groww exchange from TradeOS exchange code

        Args:
            tradeos_exchange: TradeOS exchange code (e.g., 'NSE', 'NFO')

        Returns:
            str: Groww exchange code
        """
        mapping = cls.EXCHANGE_MAP.get(tradeos_exchange, {})
        return mapping.get("exchange", tradeos_exchange)

    @classmethod
    def get_segment(cls, tradeos_exchange: str) -> str:
        """
        Get Groww segment from TradeOS exchange code
    ...
    @classmethod
    def get_exchange_segment(cls, tradeos_exchange: str) -> tuple:
        """
        Get both exchange and segment from TradeOS exchange code

        Args:
            tradeos_exchange: TradeOS exchange code

        Returns:
            tuple: (exchange, segment)
        """
        mapping = cls.EXCHANGE_MAP.get(tradeos_exchange, {})
        return mapping.get("exchange", tradeos_exchange), mapping.get("segment", "CASH")
