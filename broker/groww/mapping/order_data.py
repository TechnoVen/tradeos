...
def map_order_data(order_data):
    """
    Processes and modifies order data from Groww format to TradeOS format.

    Parameters:
    - order_data: A dictionary with either 'data' key or raw Groww API response with 'order_list'.

    Returns:
    - The modified order_data with standardized fields in TradeOS format.
    """
    ...
    for i, order in enumerate(orders_to_process):
        ...
        # Convert broker symbol to TradeOS format
        tradeos_symbol = broker_symbol
        ...
        if exchange == "NFO" and broker_symbol and " " in broker_symbol:
            try:
                ...
                from broker.groww.database.master_contract_db import format_groww_to_tradeos_symbol

                tradeos_symbol = format_groww_to_tradeos_symbol(broker_symbol, exchange)
                logger.info(f"Transformed display symbol: {broker_symbol} -> {tradeos_symbol}")
            except Exception as e:
                logger.error(f"Error converting symbol format: {e}")

        # Look up in database as fallback
        if tradeos_symbol == broker_symbol and " " in broker_symbol:
            try:
                # Look up in database
                ...
                if db_record and db_record.symbol:
                    tradeos_symbol = db_record.symbol
                    logger.info(f"Found symbol in database: {broker_symbol} -> {tradeos_symbol}")
            except Exception as e:
                logger.error(f"Error looking up symbol in database: {e}")

        mapped_order = {
            "orderid": order.get("groww_order_id", ""),
            "symbol": tradeos_symbol,  # Using the converted TradeOS format symbol
        ...
        # Map product type to TradeOS format
...
def transform_order_data(orders):
    """
    Transform order data from Groww API format to TradeOS standard format

    Args:
        orders (dict): Order data from Groww API

    Returns:
        list: Transformed orders in TradeOS format for orderbook.py
    """
    ...
    for i, order in enumerate(orders_to_process):
        ...
        # Get proper TradeOS symbol from database using token lookup
        token = None
        ...
        # If we have a token or brsymbol (tradingsymbol/trading_symbol), look up the TradeOS symbol from the database
        try:
            from database.token_db import get_oa_symbol

            # Try to get the TradeOS symbol using the token if available
            if token:
                tradeos_symbol = get_oa_symbol(token, exchange)
                if tradeos_symbol:
                    symbol = tradeos_symbol
                    logger.info(f"Found TradeOS symbol by token: {broker_symbol} -> {symbol}")

            # If token lookup failed or token wasn't available, try by broker symbol
            elif broker_symbol:
                # First check if we already have the TradeOS symbol
                if exchange == "NFO" and (
                    broker_symbol.endswith("CE") or broker_symbol.endswith("PE")
                ):
                    # Query the database to find the TradeOS symbol for this broker symbol
                    from broker.groww.database.master_contract_db import SymToken, db_session
                    ...
                    with db_session() as session:
                        ...
                        if record and record.symbol:
                            ...
                            logger.info(
                                f"Found TradeOS symbol in database: {broker_symbol} -> {symbol}"
                            )
        except Exception as e:
            logger.error(f"Error looking up TradeOS symbol from database: {e}")
        ...
        # Map order type to TradeOS format
        ...
        # For NFO instruments, ensure the symbol is in TradeOS format (AARTIIND29MAY25630CE)
        exchange = order.get("exchange", "NSE")
        if exchange == "NFO" and " " in symbol:
            try:
                ...
                tradeos_symbol = format_groww_to_tradeos_symbol(symbol, exchange)
                if tradeos_symbol:
                    # Store broker symbol for reference
                    broker_symbol = symbol
                    # Use TradeOS symbol format for display
                    symbol = tradeos_symbol
                    logger.info(f"Transformed order symbol for UI: {broker_symbol} -> {symbol}")
            except Exception as e:
                logger.error(f"Error converting order symbol format: {e}")

        # Create transformed order in TradeOS format
        transformed_order = {
            "symbol": symbol,  # Now guaranteed to be in TradeOS format
            "exchange": order.get("exchange", "NSE"),
    ...
    # Final check to ensure all symbols are in TradeOS format using database lookups
        ...
        if order.get("exchange") == "NFO" and "symbol" in order and order["symbol"]:
            ...
            if token:
                try:
                    ...
                    tradeos_symbol = get_oa_symbol(token, order.get("exchange", "NSE"))
                    if tradeos_symbol:
                        order["symbol"] = tradeos_symbol
                        logger.info(f"Final token lookup: {symbol} -> {tradeos_symbol}")
                        continue
                except Exception as e:
...
def transform_tradebook_data(tradebook_data):
    ...
    for trade in tradebook_data:
        ...
        # Try to get TradeOS symbol from database
        try:
            from database.token_db import get_oa_symbol
