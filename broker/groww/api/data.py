...
class BrokerData:
    ...
    def _convert_tradeos_to_groww_derivative_symbol(self, symbol):
        """
        Convert TradeOS NFO/BFO symbol format to Groww format
    ...
    def _convert_to_groww_params(self, symbol, exchange):
        ...
        if exchange in ["NFO", "BFO"]:
            ...
            if br_symbol:
                ...
                trading_symbol = self._convert_tradeos_to_groww_derivative_symbol(symbol)
    ...
    def get_history(
        ...
        try:
            ...
            if not df.empty:
                ...
                # NOTE: For TradeOSXTS, return non-indexed DataFrame with timestamp as a column
    ...
    def get_intervals(self) -> dict[str, dict[str, list[str]]]:
        """
        Get supported timeframes for Groww historical data in the TradeOS format.
        ...
        # Return in the standard TradeOS format
    ...
    def get_quotes(self, symbol_list, timeout: int = 5) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Quote data in TradeOS format
        """
        ...
        for sym in symbol_list:
            try:
                ...
                # Map TradeOS exchange to Groww exchange format
                if exchange == "NSE":
                    groww_exchange = EXCHANGE_NSE
                    segment = SEGMENT_CASH
                elif exchange == "BSE":
                ...
                try:
                    ...
                    if response and not response.get("error"):
                        ...
                        if response.get("status") == "SUCCESS" and isinstance(
                            ...
                            # Create quote_item in TradeOS format
                            ...
                            # CRITICAL: Build the quote item directly with values extracted from the response, using field names that TradeOS understands
        ...
        # Single symbol case - return in simpler format for TradeOS frontend
    ...
    def _format_single_quote_response(self, quote_data):
        """Helper method to convert from standard dict to the format expected by TradeOS frontend
        ...
        logger.debug(f"Final TradeOS quote format (data only): {result}")
        return result
        ...
        logger.info(f"Formatting single quote response for TradeOS frontend: {quote}")

        # Based on the sample response, TradeOS expects exactly these fields
        ...
        # LTP - preferred field name in TradeOS
        ...
        # Return exact structure expected by TradeOS
        result = {"status": "success", "data": simple_data}

        logger.info(f"FINAL FORMATTED RESULT: {result}")
        logger.info(f"Formatted result for TradeOS frontend: {result}")
    ...
    def get_depth(self, symbol_list, timeout: int = 5) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Market depth data in TradeOS format
        """
        ...
        try:
            ...
            # Create a properly formatted response for TradeOS
            depth_response = {}
            ...
            # Format the depth response according to TradeOS requirements
            depth_response = {
                "bids": bids,
                "asks": asks,
                "ltp": ltp,
    ...
    def _process_quotes_batch(self, symbols: list) -> list:
        ...
        for item in symbols:
            ...
            try:
                ...
                # For derivatives, convert symbol format
                if exchange in ["NFO", "BFO"]:
                    br_symbol = self._convert_tradeos_to_groww_derivative_symbol(br_symbol)
