        ...
        for order in orders:
            ...
            if symbol and exchange:
                ...
                if oa_symbol:
                    ...
                    # Map product types to TradeOS constants following Angel pattern
...
def transform_order_data(orders):
    ...
    for order in orders:
        ...
        # Map DefinedGe order status to TradeOS format
        status = order.get("order_status", "").upper()
        if status in ["COMPLETE", "EXECUTED"]:
...
def map_trade_data(trade_data):
    ...
    if trades:
        for trade in trades:
            ...
            # Map product types to TradeOS constants (following TradeOS order constants)
...
def transform_tradebook_data(tradebook_data):
    """
    Transform DefinedGe tradebook data to TradeOS format.

    Parameters:
    - tradebook_data: List of trade dictionaries from DefinedGe API

    Returns:
    - List of transformed trade dictionaries matching TradeOS format
    """
    transformed_data = []
    ...
    for trade in tradebook_data:
        ...
        transformed_trade = {
            "symbol": trade.get("tradingsymbol", ""),
            "exchange": trade.get("exchange", ""),
            "product": product_type,  # Mapped to TradeOS constants
...
def map_position_data(position_data):
    ...
    if positions:
        for position in positions:
            ...
            # Map product types to TradeOS constants
...
def transform_positions_data(positions_data):
    """
    Transform DefinedGe positions data to TradeOS format.
...
def transform_holdings_data(holdings_data):
    """
    Transform DefinedGe holdings data to TradeOS format.
    Following Angel's pattern for consistency.
    """
    transformed_data = []
