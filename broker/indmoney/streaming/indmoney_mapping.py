...
class IndmoneyExchangeMapper:
    """Maps TradeOS exchange codes to INDmoney-specific segment codes"""
...
class IndmoneyModeMapper:
    ...
    # Mode mapping: TradeOS mode number -> INDmoney mode string
    MODE_MAP = {
    ...
    def get_indmoney_mode(mode):
        """
        Convert TradeOS mode number to INDmoney mode string

        Args:
            mode (int): TradeOS mode (1: LTP, 2: Quote)
    ...
    @staticmethod
    def get_tradeos_mode(indmoney_mode):
        """
        Convert INDmoney mode string to TradeOS mode number

        Args:
            indmoney_mode (str): INDmoney mode ('ltp' or 'quote')

        Returns:
            int: TradeOS mode number
        """
