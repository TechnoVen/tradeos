...
class BrokerData:
    ...
    def _format_quote(self, quote_data: dict, symbol: str, exchange: str) -> dict:
        """Format quote data from Tradejini to TradeOS standard format"""
        try:
            logger.debug(f"Formatting quote data for {symbol}")

            # Extract values with defaults - matching TradeOS format
            ...
            # Format the quote to match TradeOS response exactly
            formatted_quote = {
                "ask": ask,
                "bid": bid,
                "high": high,
            ...
            # Return minimal valid quote data in TradeOS format
            return {
                "ask": 0.0,
                "bid": 0.0,
                "high": 0.0,
    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            # If no data received, return default quote in TradeOS format
    ...
    def _format_depth(self, depth_data: dict, symbol: str, exchange: str) -> dict:
        """Format depth data from Tradejini to TradeOS standard format"""
        try:
            ...
            # Format bids (buy orders) - TradeOS format (no 'orders' field)
            bids = []
            ...
            # Format asks (sell orders) - TradeOS format (no 'orders' field)
            asks = []
            ...
            # Format exactly like TradeOS sample
            formatted_depth = {
                "asks": asks,
                "bids": bids,
                "high": high,
    ...
    def _get_default_depth(self) -> dict:
        """Return default depth structure in TradeOS format"""
        return {
    ...
    def get_history(
        ...
        try:
            ...
            # Convert to TradeOS format with timestamp in seconds
            result_data = []
