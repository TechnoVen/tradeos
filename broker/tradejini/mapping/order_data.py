...
def transform_order_data(orders):
    """
    Processes and modifies a list of order dictionaries into the final TradeOS format.

    Parameters:
    - orders: List of orders with modified fields

    Returns:
    - Dictionary with orders in TradeOS format
    """
    ...
    for order in orders:
        # Convert to TradeOS format if needed
        transformed_order = {
...
def map_trade_data(trade_data):
    ...
    # Handle TradeOS format with status and data fields
    if (
    ...
    if trades_data:
        for trade in trades_data:
            ...
            mapped_trade = {
                ...
                "sym_id": symbol.get("id", ""),  # Store symbol ID for TradeOS lookup
            }
...
def transform_tradebook_data(trades):
    """
    Transforms mapped trade data to TradeOS format.

    Args:
        trades: List of mapped trade dictionaries or raw API response

    Returns:
        dict: Trade book data in TradeOS format with {'data': [...], 'status': 'success'}
    """
    logger.debug(f"transform_tradebook_data - Input trades type: {type(trades)}")

    # Check if already in TradeOS format
    if isinstance(trades, dict) and "status" in trades and "data" in trades:
        logger.debug("transform_tradebook_data - Already in TradeOS format")
    ...
    for trade in trades:
        ...
        # Get TradeOS symbol if possible
        try:
            tradeos_symbol = get_oa_symbol(symbol=sym_id, exchange=exchange)
...
def map_position_data(position_data):
    ...
    for position in position_data:
        try:
            ...
            # Get TradeOS symbol - follow same approach as the main implementation
            tradeos_symbol = None
            try:
...
def transform_positions_data(positions_data):
    """
    Transform mapped position data to TradeOS format.
    DEPRECATED: This function is kept for backward compatibility only.
...
def transform_holdings_data(holdings_data):
    """
    Transforms Tradejini holdings data to TradeOS format.
