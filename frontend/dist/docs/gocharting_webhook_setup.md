...
GoCharting is a professional HTML5 charting platform optimized for Indian markets. With a **Premium Plan**, you can set up webhook alerts to automate your trading strategies with TradeOS.
...
### 2. TradeOS Setup
- TradeOS instance running and accessible
- API key configured in TradeOS
...
If you're running TradeOS locally (on your computer) and want to test webhooks, you'll need to expose your local server to the internet using a tunneling service.
...
1. **Login to TradeOS**
   - Navigate to your TradeOS dashboard
...
5. **Configure Delivery Methods**
   ...
   - ☑️ **Webhook**: Enable this for TradeOS integration

   **Webhook URL**: Paste your TradeOS webhook URL
   ```
   https://yourdomain.com/api/v1/placeorder
   ```
...
1. **Enable Sandbox Mode in TradeOS**
...
1. Enable **Analyze Mode** in TradeOS (toggle in navbar)
2. Create a test alert in GoCharting with a condition that will trigger immediately
3. Check the **API Analyzer** in TradeOS to see the incoming request
...
```json
{
  "apikey": "your_api_key_here",      // Your TradeOS API key
...
|-------|----------|-------------|--------------|
| apikey | Yes | Your TradeOS API key | String |
...
4. **Test tunnel accessibility**
   ...
   - You should see TradeOS login page
...
### 3. Monitor Activity
- Regularly check order logs in TradeOS
...
TradeOS has rate limiting to prevent abuse:
...
### Documentation
- TradeOS Docs: https://docs.tradeos.in
...
A: Check the Logs section in TradeOS or use the API Analyzer in Analyze Mode.
...
**Happy Trading with GoCharting and TradeOS! 🚀**
