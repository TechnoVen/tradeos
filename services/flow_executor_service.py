...
Executes workflow nodes using internal TradeOS services (synchronous Flask version)
"""
...
from services.flow_tradeos_client import FlowTradeOSClient, get_flow_client
...
class NodeExecutor:
    ...
    def __init__(self, client: FlowTradeOSClient, context: WorkflowContext, logs: list):
        self.client = client
    ...
    def execute_subscribe_ltp(self, node_data: dict) -> dict:
        ...
        Connects to TradeOS WebSocket server and subscribes to LTP updates.
    ...
    def execute_subscribe_quote(self, node_data: dict) -> dict:
        ...
        Connects to TradeOS WebSocket and subscribes to quote updates (OHLC + volume).
        Falls back to REST API if WebSocket fails or times out.
        """
        symbol = self.get_str(node_data, "symbol", "")
        exchange = self.get_str(node_data, "exchange", "NSE")
    ...
    def execute_subscribe_depth(self, node_data: dict) -> dict:
        ...
        Connects to TradeOS WebSocket and subscribes to depth updates (order book).
        Falls back to REST API if WebSocket fails or times out.
        """
        symbol = self.get_str(node_data, "symbol", "")
        exchange = self.get_str(node_data, "exchange", "NSE")
